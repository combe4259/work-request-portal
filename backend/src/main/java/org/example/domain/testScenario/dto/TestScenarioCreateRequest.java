...
public record TestScenarioCreateRequest(
        @Schema(example = "로그인 성공 시나리오 검증") String title,
        @Schema(example = "정상 자격증명으로 로그인 시 JWT 토큰이 발급되는지 확인합니다.") String description,
        @Schema(example = "기능", allowableValues = {"기능", "성능", "보안", "회귀", "기타"}) String type,
        @Schema(example = "높음", allowableValues = {"높음", "보통", "낮음"}) String priority,
        @Schema(example = "작성중", allowableValues = {"작성중", "검토중", "승인됨", "실행중", "통과", "실패", "보류"}) String status,
        ...
        @Schema(example = "1") Long createdBy
) {
}
